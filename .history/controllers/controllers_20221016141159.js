...
const scheduleMeeting1 = async (req,res)=>{
   ...
   meetingEnd = moment(meetingEnd,'YYYY-MM-DD hh:mm:ss a')
    ...
    Dates.create({
        name:"randomdate",
        Date:moment("2022/10/25 10:10").format('YYYY-MM-DDTHH:mm:ss')
    }).then((datesfromdb)=>{
        console.log(moment("2022/10/25 10:10").format('YYYY-MM-DD hh:mm'))
        console.log(moment(datesfromdb).format('YYYY-MM-DDTHH:mm:ss'));
    })
    .catch((err)=>console.log(err))
