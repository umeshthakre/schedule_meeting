...
const scheduleMeeting1 = async (req,res)=>{
   ...
   meetingEnd = moment(meetingEnd,'YYYY-MM-DD hh:mm:ss a')
    ...
    console.log(moment(allUser[1].meetings[0].meetingStart))
    

    

    for(let i = 0;i<allUser.length;i++){
        let meetings = allUser[i].meetins;

        
        for(let j = 0;j<meetings.length;j++){

            console.log("start",moment(meetingStart));
            console.log("start from db",moment(meetings[i].meetingStart))
            
        
            if(
                moment(meetingStart).isBetween(moment(meetings[i].meetingStart),moment(meetings[i].meetingEnd))
                 ||
                moment(meetingEnd).isBetween(moment(meetings[i].meetingStart),moment(meetings[i].meetingEnd))
                ||
                moment(meetings[i].meetingStart).isBefore(moment(meetingStart))
                 &&
                moment(meetings[i].meetingEnd).isAfter(moment(meetingEnd))
                ){ 
                    console.log("cmon")
                return res.send({success:false, message:`${allUser[i]} has a meeting ${meetings[j]}`})
            }
